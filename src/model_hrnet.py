...
class HighResolutionNet(nn.Module):
    ...
    def init_weights(self, pretrained=''):
        ...
        if os.path.isfile(pretrained):
            ...
            if torch.cuda.is_available():
                ...
                location='cpu'
            pretrained_dict = torch.load(pretrained, map_location=torch.device(location))
    ...
    def load_state_dict_HR(self, pretrained_dict):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                # nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                nn.init.normal_(m.weight, std=0.001)
...
class Vanillanet(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=2, padding=1,
                               bias=False)
        # self.bn1 = BatchNorm2d(64, momentum=BN_MOMENTUM)
        self.conv2 = nn.Conv2d(64, 68, kernel_size=3, stride=2, padding=1,
                               bias=False)
        # self.bn2 = BatchNorm2d(68, momentum=BN_MOMENTUM)
        self.relu = nn.ReLU(inplace=True)
